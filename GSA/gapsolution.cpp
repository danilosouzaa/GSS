...
struct JobsCompAsc
{
    ...
    JobsCompAsc(GAPJobData  *ref) {
        jobsRef = ref;
    }
    bool operator()(GAPJobData *j1, GAPJobData *j2, uint *idAgent) { //two Jobs one agents
        return jobsRef[j1->id]->costAgentsAllocation[idAgent] < jobsRef[j2->id]->costAgentsAllocation[idAgent];
    }
};
...
GAPSolution::GAPSolution(GAProblem &prob, int flags) : problem(prob)
{
...
{
    id = idGen++;
    sizeJobs = problem.nJobs;
    sizeAgents = problem.mAgents;

    jobs  = new ushort[2 * sizeJobs];
    cost  = jobs + sizeJobs;
    resource = cost + sizeJobs;
    ...
    adsRowElems = GPU_BLKSIZE_GLOBAL(sizeJobs * sizeof(uint)) / sizeof(uint);
#else
    adsRowElems = sizeJobs;
#endif

    adsDataSize = ADS_INFO_SIZE + adsRowElems * (sizeJobs + 3) * sizeof(uint);
...
void
GAPSolution::update() // seta vetor weights (distancia entre no anterior e atual)
{
    uint    k;
    ...
    for(uint i=1;i < jobsCount;i++,k--) {
        idAgentAllocated = jobs[i];
        cost +=  problem.getCostJobsAgents(i,idAgent);
    }
}
...
uint
GAPSolution::showCostCalc(const char *prompt)
{
//     uint     ccost;
//     uint     i,k;
...
void
GAPSolution::assign(GALSolution *sol, bool eval)
{
	if(this == sol)
		return;
	...
	time = sol->time;
	cost = sol->cost;
	jobsCount = sol->jobsCount;
	memcpy(jobs,sol->jobs,dataSize);
...
void
GAPSolution::adsAssign(const GAPSolution &sol)
{
	if(sol.adsData) {
		if(adsData == NULL)
			adsAlloc();
...
ullong
GAPSolution::ldsChecksum(uint max)
{
    ullong  sum;
    gpubyte   *data;
...
void
GAPSolution::showWeight(ostream &os)
{
    // uint i,j,n,w;
    ...
    //     os << setw(w) << i << '|';
    //     for(j=0;j < n;j++)
    //         os << setw(w) << problem.jobs[i].weight[j];
    //     os << '\n';
    // }
    // os << endl;
}
...
void
GAPSolution::ldsShow(MLADSField field, ostream &os)
{
    const
	char        *label = "WTCD";
    uint        *adsRow,
...
CompAsc
{
    ...
    ClientCompAsc(GAPClientData  *ref) {
        clRef = ref;
    }
    bool operator()(GAPClientData *c1, GAPClientData *c2) {
...
struct ClientCompDsc
{
    ...
    ClientCompDsc(GAPClientData  *ref) {
        clRef = ref;
    }
    bool operator()(GAPClientData *c1, GAPClientData *c2) {
        return clRef->weight[c1->id] > clRef->weight[c2->id];
...
GAPSolution::GAPSolution(GAPProblem &prob, int flags) : problem(prob)
{
	// TODO: verificar com Eyder!!
    //if(problem.params.pinnedAlloc)
    //    adsFlags = flags;
...
void
GAPSolution::assign(GAPSolution *sol, bool eval)
{
	if(this == sol)
		return;
...
void
GAPSolution::ldsShow(GAPADSField field, ostream &os)
{
    const
	char        *label = "WTCD";
    uint        *adsRow,
...
void
GAPSolution::checkCost(const char *prompt)
{
	uint    ccost;
	uint    ids[clientCount];
	uint    i;
...
void
GAPSolution::save(const char *fname)
{
    ofstream os(fname,ios_base::trunc);
...
void
GAPSolution::load(const char *fname)
{
    ifstream is(fname);
...
void
GAPSolution::random(MTRandom &rng, float alpha)
{
    PGAPClientData  clData[problem.size],// buffer for candidates list
                  *clCands;             // candidates list (CL)
    GAPClientData  *clLast,              // last client added to solution
                  *clCurr;              // client being added to solution
    uint           clCount,             // number of candidates at CL
                   rclCount,            // number of candidates at RCL
                   rclIndex;            // index of an RCL element
