...
__global__ void  SA_GPU(Instance *inst, Solution *sol, Moviment *mov, unsigned int *seed, curandState_t* states, int iteration, int n_busca){
	register int i, j;
	register unsigned short int nJobs = inst->nJobs;
    register unsigned short int mAgents = inst->mAgents;
	register unsigned short int resUsageTemp[800];//MAX AGENTS INSTANCE 
	__shared__ unsigned short int s_shared[16000];//MAX JOBS INTANCES
	#define term  threadIdx.x + blockIdx.x*nThreads
	#define blkIdMAgents blockIdx.x*mAgents
	...
	for(i = 0; i<= nJobs/nThreads; ++i){ 
		if(threadIdx.x <  nJobs - i*nThreads){   //define que somente as primeiras thread do bloco serão responsáveis por realizar a copia da solução inicial no seu bloco
	...
	for(int s_search = 0; s_search < n_busca; ++s_search){
		do{
			for(i=0; i < mAgents; ++i){
				res_aux[i] = sol->resUsage[i + blockIdx.x*mAgents]; 
			}
			int optNeighborhood = 1;
			if(optNeighborhood == 1){
				register short int swapJobSrc = curand(&states[term])%nJobs;
				register unsigned short int swapAgentDst = curand(&states[term])%mAgents;
				if(sol->resUsage[swapAgentDst + blkIdMAgents] + inst->resourcesAgent[swapJobSrc*mAgents + swapAgentDst] <= inst->capacity[swapAgentDst]){
					//if viable move?
					// delta_aux < 0: improment, delta_aux > 0 :worsening
					delta_aux[term] = inst->cost[swapJobSrc*mAgents +swapAgentDst] - inst->cost[swapJobSrc*mAgents + s_shared[swapJobSrc]];
					if(delta_aux[term]<0){

					}
				}

			}else if(optNeighborhood==2){
