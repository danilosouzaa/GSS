...
#ifndef __gapsolution_h
#define __gapsolution_h
 ...
 */
enum GAPRouteType {
        GAPRT_PATH,      ///< PATH: don't returns to depot after visit last client
        GAPRT_TOUR,      ///< TOUR: returns to depot after visit last client
};
...
public:
    GAProblem        &problem;                  ///< Optimization problem
    ...
    uint              jobsCount;              ///< Number of jobs in solution
    ushort           *jobs;                  ///< Solution Jobs
    ushort           *costAllocation;                  ///< Weight between solution jobs
    ushort           *resource;             ///< resource
    uint              dataSize;                 ///< Solution data size in bytes

    GAPMove            move;                     ///< Optional movement related to solution

    GAPADSData        *adsData;                  ///< Evaluation data
...
protected:
     ...
     */
    void
    adsAssign(const GAPSolution &sol);
    /*!
...
public:
    /*!
     * Create a GAPP solution instance.
     */
    GAPSolution(GAProblem &prob, int flags = cudaHostAllocDefault);
    /*!
     * Destroy a GAPP solution instance.
     */
    virtual
    ~GAPSolution();
    /*!
    ...
    costTypeName();
    /*!
     * Calculate solution cost based on jobs route. The current solution's cost is not
     * Updated.
     *
    ...
    int
    coordy(uint i) {
        return problem.jobs[ jobs[i] ].y;
    }
    /*!
     * Get distance between two solution jobs.
     */
    inline
    int
    dist(uint i, uint j) {
        return problem.jobs[ jobs[i] ].weight[ jobs[j] ];
    }
    /*!
     * Get distance between two solution jobs.
     */
    ...
    calcDist(uint i, uint j) {
        return sqrtf( sqr(problem.jobs[ jobs[i] ].x - problem.jobs[ jobs[j] ].x) +
                      sqr(problem.jobs[ jobs[i] ].y - problem.jobs[ jobs[j] ].y) );
    }
    /*!
    ...
    inline
    bool
    add(GAPJobsData *cdata) {

#if LOG_LEVEL > 0
        if(clientCount >= size) {
        	WARNING("No more room for jobs in solution.");
        	return false;
        }
#endif
        jobs[clientCount++] = cdata->id;
...
#if LOG_LEVEL > 0
        if(clientCount >= size)
        	EXCEPTION("No more room for jobs in solution.");
#endif
        jobs[clientCount++] = id;
     ...
     */
    void
    assign(GAPSolution *sol, bool eval);
    /*!
     * Assign a solution.
     *
     * @param   sol     Solution to be assigned
    ...
    /*!
     * Calculate solution cost based on jobs route showing result.
     ...
     */
    void
    ldsShow(GAPADSField field, std::ostream &os = std::cout);

    void ldsShow(std::ostream &os = std::cout) {
    	os << "LDS SHOW BEGIN" << std::endl;
    	for(int f=GAPAF_COORD; f<=GAPAF_COST; f++)
    		ldsShow((GAPADSField)f,os);
    	os << "LDS SHOW END" << std::endl;
    }

    /*!
     * Get eval data travel time between two jobs.
     */
    inline
    uint
    ...
    inline
    bool
    operator<(GAPSolution &sol) {
        return cost < sol.cost;
    }
    /*!
     * Friend classes
     */
    friend class GAPProblem;
    //friend class GAPGPUTask;
};

// std::ostream &operator<<(std::ostream &os, GAPSolution const &s);
